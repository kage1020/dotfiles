    ...
    if projects_dir.exists():
        for project_dir in projects_dir.iterdir():
            ...
            project_name_from_dir = (
                dir_name.rsplit("-", 1)[-1] if "-" in dir_name else dir_name
            )
            if (
                project_filter
                and project_filter.lower() not in project_name_from_dir.lower()
            ):
...
def collect_copilot_chat(cutoff_ms: int | None, project_filter: str | None) -> dict:
    ...
    if all_prompts:
        ...
        timestamps = [
            m["timestamp"] for m in all_prompts if m["timestamp"] != "unknown"
        ]
        if timestamps:
            result["period"] = f"{min(timestamps)} 〜 {max(timestamps)}"
                        ...
                        if workspace_json.exists():
                            try:
                                ...
                                if folder.startswith("file://"):
                                    folder = folder[7:]
                                project_name = (
                                    Path(folder).name if folder else project_name
                                )
...
def collect_cline(cutoff_ms: int | None) -> dict:
    ...
    all_prompts = []
    task_dirs = sorted(
        tasks_dir.iterdir(), key=lambda p: p.stat().st_mtime, reverse=True
    )[:20]
...
def collect_windsurf(cutoff_ms: int | None) -> dict:
    ...
    all_entries = []
    for mem_file in sorted(
        memories_dir.rglob("*"), key=lambda p: p.stat().st_mtime, reverse=True
    )[:20]:
    ...
    if all_entries:
        ...
        timestamps = [
            m["timestamp"] for m in all_entries if m["timestamp"] != "unknown"
        ]
        if timestamps:
            result["period"] = f"{min(timestamps)} 〜 {max(timestamps)}"
    ...
    for rollout_path in rollout_files:
        ...
        try:
            ...
            if project_filter:
                ...
                if filter_lower not in project_name.lower() and (
                    not cwd or filter_lower not in cwd.lower()
                ):
                    continue
    ...
    for db_path in db_paths:
        ...
        try:
            ...
            for message_id, entry in grouped.items():
                ...
                project_name = (
                    Path(project_source).name if project_source else "unknown"
                )
                ...
                messages.append(
                    {
                        "text": text[:500],
                        "timestamp": ts_to_iso(timestamp_ms)
                        if timestamp_ms
                        else "unknown",
...
def main():
    ...
    parser.add_argument(
        "--days", type=int, default=7, help="過去N日分に限定（デフォルト: 7日）"
    )
    parser.add_argument(
        "--project", type=str, default=None, help="プロジェクト名でフィルタ（部分一致）"
    )
